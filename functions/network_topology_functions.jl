...
for i in 1:size(RES_bus,1)
    if nTP==1
prof_PRES[:,:,i]=RES_MAG*RES_cap[i]*data_scen[1:nSc,which_hour]
else
...
load_dict=Dict{Int64,Array{Float64,1}}()
for i in 1:size(nw_pPrf_data_load[:,1],1)
    push!(load_dict, nw_pPrf_data_load[i,1]=>nw_pPrf_data_load[i,2:end] )
end
EV_dict=Dict{Int64,Array{Float64,1}}()
for i in 1:size(rdata_EVProfile_load[:,1],1)
    push!(EV_dict, rdata_EVProfile_load[i,1]=>rdata_EVProfile_load[i,2:end] )
end
for i in union(keys(load_dict),keys(EV_dict))
    if haskey(load_dict, i) && haskey(EV_dict, i)
        push!(load_dict, i=> values(load_dict[i])+values(EV_dict[i]) )
    elseif haskey(load_dict, i) && !haskey(EV_dict, i)
        push!(load_dict, i=> values(load_dict[i]) )
    elseif !haskey(load_dict, i) && haskey(EV_dict, i)
        push!(load_dict, i=> values(EV_dict[i]) )
    end
end
...
nLoads = length(prof_ploads[:,1])
# prof_ploads=0*prof_ploads
# prof_ploads=replace(x -> x<0 ? 0 : x, prof_ploads)
bus_data_lsheet       = nw_pPrf_data_load[:,1]
# ================================reactive power ===============
prof_qloads =Load_MAG*nw_qPrf_data_load[:,2:end]*load_correction
nw_qPrf_data_load= hcat(load_bus_map,prof_qloads)
prof_EV_q     = tan(acos(power_factor))*rdata_EVProfile_load[:,2:end]
prof_EV_q     =0*rdata_EVProfile_load[:,2:end]
println("")
println("")
show("NO reactive power for EV nodes!")
println("")
rdata_EVProfile_loadq= hcat(rdata_EVProfile_load[:,1],prof_EV_q)



qload_dict=Dict{Int64,Array{Float64,1}}()
for i in 1:size(nw_qPrf_data_load[:,1],1)
    push!(qload_dict, nw_qPrf_data_load[i,1]=>nw_qPrf_data_load[i,2:end] )
end
qEV_dict=Dict{Int64,Array{Float64,1}}()
for i in 1:size(rdata_EVProfile_loadq[:,1],1)
    push!(qEV_dict, rdata_EVProfile_loadq[i,1]=>rdata_EVProfile_loadq[i,2:end] )
end
for i in union(keys(qload_dict),keys(qEV_dict))
    if haskey(qload_dict, i) && haskey(qEV_dict, i)
        push!(qload_dict, i=> values(qload_dict[i])+values(qEV_dict[i]) )
    elseif haskey(qload_dict, i) && !haskey(qEV_dict, i)
        push!(qload_dict, i=> values(qload_dict[i]) )
    elseif !haskey(qload_dict, i) && haskey(qEV_dict, i)
        push!(qload_dict, i=> values(qEV_dict[i]) )
    end
end
...
return    include_new#,node_data_updated
...
function load_data(filename_prof)
#     sheetname  = "Loads";
...
cost_flex_load = repeat([ev_cost][1],nEV)
nFl=nEV
nd_fl=nd_EV
...
penalty_cost=5*maximum(cost_b_gen)*sbase
return  Pg_max,Pg_min,Qg_max,Qg_min,cA_gen,cB_gen,cC_gen,bus_data_gsheet,pg_max,pg_min,qg_max,qg_min,cost_a_gen,cost_b_gen,cost_c_gen,penalty_cost
end
