...
function variables_n(model_name)
      ...
      # variable_by_name(model_name, "e[1,1,1]")  all of e[1,1,1] is the name
         ...
         p_fl_dec=nothing
           if nFl!=0
               @variable(model_name, p_fl_inc[ j = 1:nTP, k = 1:nFl], start=0.5*load_inc_prct*prof_ploads[k,j],lower_bound=0.0, upper_bound=ev_od[j])
               @variable(model_name, p_fl_dec[ j = 1:nTP, k = 1:nFl], start=0.5*load_dec_prct*prof_ploads[k,j],lower_bound=0.0, upper_bound=-ev_ud[j])
          end
          p_ch=nothing
          p_dis=nothing
          soc=nothing
      ...
      return e, f, Pg, Qg, pen_ws, pen_lsh, p_fl_inc, p_fl_dec, p_ch, p_dis, soc
...
function variables_c(model_name)
         ...
         @variable(model_name,  f_c[c = 1:nCont,i = 1:nSc, j = 1:nTP, k = 1:nBus], start=0.0)
          ...
          p_fl_dec_c=nothing
          if   nFl!=0
              @variable(model_name, p_fl_inc_c[c = 1:nCont,i = 1:nSc, j = 1:nTP, k = 1:nFl], start=0.5*load_inc_prct*prof_ploads[k,j],lower_bound=0.0, upper_bound=ev_od[j])
              @variable(model_name, p_fl_dec_c[c = 1:nCont,i = 1:nSc, j = 1:nTP, k = 1:nFl], start=0.5*load_dec_prct*prof_ploads[k,j],lower_bound=0.0, upper_bound=-ev_ud[j])
       ...
       return  e_c, f_c, Pg_c, Qg_c, pen_ws_c, pen_lsh_c, p_fl_inc_c ,p_fl_dec_c, p_ch_c, p_dis_c, soc_c
...
return line_flow_normal_s,line_flow_normal_r
...
return line_flow_contin_s,line_flow_contin_r
...
function objective_SCOPF(model_name)
